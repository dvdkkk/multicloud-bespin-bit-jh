...
export default function App() {
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // 즉시 낙관적 UI 적용: 서버 응답을 기다리지 않고 즉시 완료 메시지 표시 (0.1초 반응 체감)
    setIsSubmitted(true);

    // 백그라운드 전송 및 keepalive: true 옵션으로 페이지 이동이나 창 닫힘 시에도 전송 보장
  ...
  desc: (
    <>
      ...
      <div className="flex justify-center -mt-10 mb-24">
        <button 
          onClick={() => scrollTo('contact-form')}
          className="bg-yellow-400 text-blue-950 hover:bg-yellow-300 px-10 py-5 rounded-full font-black text-xl transition-all shadow-xl hover:scale-105 flex items-center justify-center gap-3 animate-pulse"
        >
            ...
            <div className="bg-white p-6 md:p-8 rounded-3xl shadow-2xl">
              {isSubmitted ? (
                <div className="py-8 px-2 text-center flex flex-col items-center justify-center">
                  <div className="w-16 h-16 bg-emerald-50 text-emerald-600 rounded-full flex items-center justify-center mb-5 shadow-lg shadow-emerald-500/10">
                  ...
                  <p className="text-slate-600 text-sm md:text-base leading-relaxed mb-8 max-w-sm">
                    문의해주셔서 감사합니다.<br />
                    작성해주신 연락처로 베스핀글로벌 전문 상담사가 신속하게 확인 후 안내해 드리겠습니다.
                  </p>

                  <div className="w-full bg-slate-50 p-4 md:p-5 rounded-2xl border border-slate-100 text-left text-xs md:text-sm text-slate-600 mb-8 space-y-2">
                    <div className="flex items-center gap-2 font-bold text-slate-800">
                      <span className="w-2 h-2 rounded-full bg-blue-600"></span>
                      안내사항
                    </div>
                    <p className="text-xs text-slate-500 leading-relaxed pl-4">
                      • 국비지원 카드 발급 방법 및 선발 절차(상담, 레벨테스트 등)를 순차적으로 안내드립니다.<br />
                      • 빠른 상담을 원하시는 경우 고객센터 운영시간(평일 09:00~18:00) 내에 연락드립니다.
                    </p>
                  </div>
                  ...
                  <button
                    ...
                    className="inline-flex items-center justify-center gap-2 px-6 py-3 rounded-xl border border-slate-200 text-slate-700 font-bold hover:bg-slate-50 hover:border-slate-300 transition-all text-sm"
                  >
                    추가 문의 작성하기
                  </button>
                </div>
                  ...
                  >
                    ...
                    <div>
                      ...
                      <textarea 
                        ...
                        placeholder="궁금하신 내용을 적어주세요. (선택 사항)"
                      ></textarea>
                    </div>
